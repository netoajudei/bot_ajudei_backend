...
CREATE OR REPLACE FUNCTION "public"."buscar_contrato"("args" "jsonb", "cliente_id" bigint DEFAULT NULL::bigint) RETURNS "jsonb"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  v_telefone text;
  ...
  v_action text;
BEGIN
  RAISE WARNING '🔵 [buscar_contrato] Iniciando - cliente_id: %, args: %', cliente_id, args;

  -- ============================================
  -- 1. VALIDAR CLIENTE_ID
  -- ============================================
  IF cliente_id IS NULL THEN
    RAISE WARNING '❌ [buscar_contrato] cliente_id é NULL';
    RETURN jsonb_build_object(
      'success', false,
  ...
  -- ============================================
  SELECT "chatId" INTO v_telefone
  FROM clientes
  WHERE id = cliente_id;

  RAISE WARNING '📞 [buscar_contrato] Telefone encontrado: %', v_telefone;

  IF v_telefone IS NULL THEN
    RAISE WARNING '❌ [buscar_contrato] Cliente % não tem telefone cadastrado', cliente_id;
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Cliente não encontrado ou sem telefone cadastrado'
    );
  END IF;

  -- ============================================
  -- 3. EXTRAIR DADOS DOS ARGS
  -- ============================================
  IF args ? 'dados' AND jsonb_typeof(args->'dados') = 'array' THEN
    v_dados := args->'dados'->0;
    RAISE WARNING '📦 [buscar_contrato] Extraindo dados do array: %', v_dados;
  ELSE
    v_dados := args;
    RAISE WARNING '📦 [buscar_contrato] Usando args direto: %', v_dados;
  END IF;

  -- ============================================
  -- 4. BUSCAR CONTRATO EXISTENTE
  -- ============================================
  ...
  IF v_contrato_id IS NULL THEN
    RAISE WARNING '🆕 [buscar_contrato] Nenhum contrato existente - vai INSERIR';
    v_action := 'created';
  ELSE
    RAISE WARNING '♻️ [buscar_contrato] Contrato existente: % - vai ATUALIZAR', v_contrato_id;
    v_action := 'updated';
  END IF;

  -- ============================================
  -- 5. UPSERT DO CONTRATO
  -- ============================================
  IF v_contrato_id IS NULL THEN
    -- ──────────────────────────────────────────
    -- INSERT
    -- ──────────────────────────────────────────
    RAISE WARNING '➕ [buscar_contrato] Executando INSERT...';
    ...
    BEGIN
      INSERT INTO contratos (
        ...
        cliente_id,
        NULLIF(v_dados->>'nome', ''),
        NULLIF(v_dados->>'numero_contrato', ''),
        NULLIF(v_dados->>'banco', ''),
        NULLIF((v_dados->>'parcelas')::text, '')::integer,
        NULLIF((v_dados->>'valor_parcela')::text, '')::numeric,
        NULLIF((v_dados->>'prazo_contrato')::text, '')::integer,
        NULLIF((v_dados->>'parcelas_pagas')::text, '')::integer,
        NULLIF((v_dados->>'parcelas_em_aberto')::text, '')::integer,
        NULLIF((v_dados->>'parcelas_atrasadas')::text, '')::integer,
        NULLIF((v_dados->>'valor_total_contrato')::text, '')::numeric,
        NULLIF((v_dados->>'valor_estimado_quitacao')::text, '')::numeric,
        NULLIF((v_dados->>'percentual_desconto')::text, '')::numeric,
      ...
      RAISE WARNING '✅ [buscar_contrato] INSERT bem-sucedido - ID: %', v_contrato_id;
      
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING '❌ [buscar_contrato] ERRO no INSERT: % - %', SQLSTATE, SQLERRM;
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Erro ao inserir contrato: ' || SQLERRM
      );
    END;

  ELSE
    -- ──────────────────────────────────────────
    -- UPDATE (SIMPLIFICADO COM COALESCE)
    -- ──────────────────────────────────────────
    RAISE WARNING '🔄 [buscar_contrato] Executando UPDATE no contrato %...', v_contrato_id;
    ...
    BEGIN
      ...
      SET
        ...
        -- Atualiza apenas se vier valor não-vazio
        nome = COALESCE(NULLIF(v_dados->>'nome', ''), nome),
        numero_contrato = COALESCE(NULLIF(v_dados->>'numero_contrato', ''), numero_contrato),
        banco = COALESCE(NULLIF(v_dados->>'banco', ''), banco),
        nome_pai = COALESCE(NULLIF(v_dados->>'nome_pai', ''), nome_pai),
        nome_mae = COALESCE(NULLIF(v_dados->>'nome_mae', ''), nome_mae),
        
        -- Campos numéricos com cast explícito
        parcelas = COALESCE(
          NULLIF((v_dados->>'parcelas')::text, '')::integer, 
          parcelas
        ),
        
        valor_parcela = COALESCE(
          NULLIF((v_dados->>'valor_parcela')::text, '')::numeric,
          valor_parcela
        ),
        
        prazo_contrato = COALESCE(
          NULLIF((v_dados->>'prazo_contrato')::text, '')::integer,
          prazo_contrato
        ),
        ...
        parcelas_em_aberto = COALESCE(
          NULLIF((v_dados->>'parcelas_em_aberto')::text, '')::integer,
          parcelas_em_aberto
        ),
        
        parcelas_atrasadas = COALESCE(
          NULLIF((v_dados->>'parcelas_atrasadas')::text, '')::integer,
          parcelas_atrasadas
        ),
        
        valor_total_contrato = COALESCE(
          NULLIF((v_dados->>'valor_total_contrato')::text, '')::numeric,
          valor_total_contrato
        ),
        
        valor_estimado_quitacao = COALESCE(
          NULLIF((v_dados->>'valor_estimado_quitacao')::text, '')::numeric,
          valor_estimado_quitacao
        ),
        
        percentual_desconto = COALESCE(
          NULLIF((v_dados->>'percentual_desconto')::text, '')::numeric,
          percentual_desconto
        ),
        
        -- Observações: concatena em vez de substituir
        observacoes = CASE
          WHEN NULLIF(v_dados->>'observacoes', '') IS NOT NULL THEN
      ...
      RAISE WARNING '✅ [buscar_contrato] UPDATE bem-sucedido';
      
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING '❌ [buscar_contrato] ERRO no UPDATE: % - %', SQLSTATE, SQLERRM;
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Erro ao atualizar contrato: ' || SQLERRM,
        'sqlstate', SQLSTATE
      );
    END;
  ...
  -- ============================================
  -- 6. BUSCAR CONTRATO ATUALIZADO
  -- ============================================
  RAISE WARNING '🔍 [buscar_contrato] Buscando contrato atualizado...';
  ...
  IF NOT FOUND THEN
    RAISE WARNING '❌ [buscar_contrato] Contrato não encontrado após INSERT/UPDATE!';
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Contrato não encontrado após operação'
    );
  END IF;

  RAISE WARNING '✅ [buscar_contrato] Sucesso! Action: %, Contrato ID: %', v_action, v_contrato_id;
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING '🔥 [buscar_contrato] ERRO GERAL: % - %', SQLSTATE, SQLERRM;
    RETURN jsonb_build_object(
      'success', false,
...
ALTER FUNCTION "public"."buscar_contrato"("args" "jsonb", "cliente_id" bigint) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."buscar_contrato"("args" "jsonb", "cliente_id" bigint) IS 'Faz UPSERT do contrato com casts explícitos e NULLIF para evitar strings vazias.
Usa COALESCE para manter valores existentes quando campo vem vazio.';
...
ALTER TABLE "public"."contrato_arquivos" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."functions" (
    ...
    "prompt_id" bigint,
    "is_rpc" boolean DEFAULT false NOT NULL
);
...
COMMENT ON COLUMN "public"."functions"."is_rpc" IS 'Define se a função é RPC (true) ou Edge Function (false). Se true, usa RPC do PostgreSQL. Se false, chama Edge Function via HTTP.';
...
ALTER TABLE ONLY "public"."contrato_arquivos"
    ADD CONSTRAINT "contrato_arquivos_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "idx_contrato_arquivos_contrato" ON "public"."contrato_arquivos" USING "btree" ("contrato_id");
...
ALTER TABLE ONLY "public"."contrato_arquivos"
    ADD CONSTRAINT "contrato_arquivos_contrato_id_fkey" FOREIGN KEY ("contrato_id") REFERENCES "public"."contratos"("id") ON DELETE CASCADE;
...
CREATE POLICY "Permitir todas operações em arquivos" ON "public"."contrato_arquivos" USING (true) WITH CHECK (true);
...
GRANT ALL ON FUNCTION "public"."buscar_contrato"("args" "jsonb", "cliente_id" bigint) TO "anon";
GRANT ALL ON FUNCTION "public"."buscar_contrato"("args" "jsonb", "cliente_id" bigint) TO "authenticated";
GRANT ALL ON FUNCTION "public"."buscar_contrato"("args" "jsonb", "cliente_id" bigint) TO "service_role";
...
GRANT ALL ON TABLE "public"."contrato_arquivos" TO "anon";
GRANT ALL ON TABLE "public"."contrato_arquivos" TO "authenticated";
GRANT ALL ON TABLE "public"."contrato_arquivos" TO "service_role";
